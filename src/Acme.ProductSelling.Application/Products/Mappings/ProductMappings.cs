...
#region Product Mappings (COMPREHENSIVE SPEC HANDLING)
...
public partial class ProductToProductDtoMapper : MapperBase<Product, ProductDto>
{
    ...
    // Safety Ignores for Logic properties
    ...
    // --- FULL SPEC IGNORE LIST (Prevents implicit generation crashes) ---
    ...
    // --- SUB MAPPERS (Explicit logic to avoid NREs) ---
    private MonitorSpecificationDto MapMonitor(MonitorSpecification s)
    {
        var d = new MonitorSpecificationDto();
        d.Id = s.Id; d.RefreshRate = s.RefreshRate; d.Resolution = s.Resolution;
        d.ScreenSize = s.ScreenSize; d.ResponseTime = s.ResponseTime; d.ResponseTimeMs = s.ResponseTimeMs;
        d.ColorGamut = s.ColorGamut; d.Brightness = s.Brightness; d.VesaMount = s.VesaMount;
        d.PanelTypeName = s.PanelType?.Name; // Checked access
        return d;
    }
    private CpuSpecificationDto MapCpu(CpuSpecification s)
    {
        var d = new CpuSpecificationDto();
        d.Id = s.Id; d.CoreCount = s.CoreCount; d.ThreadCount = s.ThreadCount; d.BaseClock = s.BaseClock;
        d.BoostClock = s.BoostClock; d.L3Cache = s.L3Cache; d.Tdp = s.Tdp; d.HasIntegratedGraphics = s.HasIntegratedGraphics;
        d.SocketName = s.Socket?.Name; // Checked access
        return d;
    }
    private MotherboardSpecificationDto MapMb(MotherboardSpecification s)
    {
        var d = new MotherboardSpecificationDto();
        d.Id = s.Id; d.RamSlots = s.RamSlots; d.MaxRam = s.MaxRam; d.M2Slots = s.M2Slots;
        d.SataPorts = s.SataPorts; d.HasWifi = s.HasWifi;
        d.SocketName = s.Socket?.Name;
        d.ChipsetName = s.Chipset?.Name;
        d.FormFactorName = s.FormFactor?.Name;
        d.SupportedRamTypeName = s.SupportedRamTypes?.Name;
        return d;
    }
    private RamSpecificationDto MapRam(RamSpecification s)
    {
        var d = new RamSpecificationDto();
        d.Id = s.Id; d.Capacity = s.Capacity; d.Speed = s.Speed; d.ModuleCount = s.ModuleCount;
        d.Timing = s.Timing; d.Voltage = s.Voltage; d.HasRGB = s.HasRGB; d.RamFormFactor = s.RamFormFactor;
        d.RamTypeName = s.RamType?.Name;
        return d;
    }
    private PsuSpecificationDto MapPsu(PsuSpecification s)
    {
        var d = new PsuSpecificationDto();
        d.Id = s.Id; d.Wattage = s.Wattage; d.EfficiencyRating = s.EfficiencyRating; d.Modularity = s.Modularity;
        d.FormFactorName = s.FormFactor?.Name;
        return d;
    }
    private KeyboardSpecificationDto MapKb(KeyboardSpecification s)
    {
        var d = new KeyboardSpecificationDto();
        d.Id = s.Id; d.KeyboardType = s.KeyboardType; d.Layout = s.Layout; d.Connectivity = s.Connectivity; d.Backlight = s.Backlight;
        d.SwitchTypeName = s.SwitchType?.Name;
        return d;
    }
    private CaseSpecificationDto MapCase(CaseSpecification s)
    {
        var d = new CaseSpecificationDto();
        d.Id = s.Id; d.Color = s.Color; d.MaxGpuLength = s.MaxGpuLength; d.MaxCpuCoolerHeight = s.MaxCpuCoolerHeight;
        d.SupportedMbFormFactorName = s.FormFactor?.Name;
        d.MaterialNames = s.Materials?.Select(m => m.Material?.Name).Where(x => x != null).ToList();
        return d;
    }
    private CpuCoolerSpecificationDto MapCooler(CpuCoolerSpecification s)
    {
        var d = new CpuCoolerSpecificationDto();
        d.Id = s.Id; d.CoolerType = s.CoolerType; d.FanSize = s.FanSize; d.RadiatorSize = s.RadiatorSize ?? 0;
        d.Height = s.Height; d.TdpSupport = s.TdpSupport; d.NoiseLevel = s.NoiseLevel; d.Color = s.Color; d.LedLighting = s.LedLighting;
        d.SupportedSocketNames = s.SupportedSockets?.Select(ss => ss.Socket?.Name).Where(x => x != null).ToList();
        return d;
    }
    // -- Safe Simple Mappings --
    ...
    private HeadsetSpecificationDto MapHeadset(HeadsetSpecification s) => new HeadsetSpecificationDto
    {
        ...
        DriverSize = s.DriverSize.ToString()
    };
    private SpeakerSpecificationDto MapSpeaker(SpeakerSpecification s) => new SpeakerSpecificationDto { Id = s.Id, SpeakerType = s.SpeakerType, TotalWattage = s.TotalWattage, Frequency = s.Frequency, Connectivity = s.Connectivity, InputPorts = s.InputPorts, Color = s.Color };
    private WebcamSpecificationDto MapWebcam(WebcamSpecification s) => new WebcamSpecificationDto { Id = s.Id, Resolution = s.Resolution, FrameRate = s.FrameRate, FocusType = s.FocusType, FieldOfView = s.FieldOfView, Connectivity = s.Connectivity, Connection = s.Connection, HasMicrophone = s.HasMicrophone, HasPrivacyShutter = s.HasPrivacyShutter, MountType = s.MountType, Color = s.Color };
    private CableSpecificationDto MapCable(CableSpecification s) => new CableSpecificationDto { Id = s.Id, CableType = s.CableType, Length = s.Length, MaxPower = s.MaxPower, DataTransferSpeed = s.DataTransferSpeed, Connector1 = s.Connector1, Connector2 = s.Connector2, IsBraided = s.IsBraided, Color = s.Color, Warranty = s.Warranty };
    private SoftwareSpecificationDto MapSoftware(SoftwareSpecification s) => new SoftwareSpecificationDto { Id = s.Id, SoftwareType = s.SoftwareType, LicenseType = s.LicenseType, Platform = s.Platform, Version = s.Version, Language = s.Language, DeliveryMethod = s.DeliveryMethod, SystemRequirements = s.SystemRequirements, IsSubscription = s.IsSubscription };
    private CaseFanSpecificationDto MapCaseFan(CaseFanSpecification s) => new CaseFanSpecificationDto { Id = s.Id, FanSize = s.FanSize, MaxRpm = s.MaxRpm, NoiseLevel = s.NoiseLevel, Airflow = s.Airflow, StaticPressure = s.StaticPressure, Connector = s.Connector, BearingType = s.BearingType, HasRgb = s.HasRgb, Color = s.Color };
    private ChairSpecificationDto MapChair(ChairSpecification s) => new ChairSpecificationDto { Id = s.Id, ChairType = s.ChairType, Material = s.Material, MaxWeight = s.MaxWeight, ArmrestType = s.ArmrestType, BackrestAdjustment = s.BackrestAdjustment, SeatHeight = s.SeatHeight, HasLumbarSupport = s.HasLumbarSupport, HasHeadrest = s.HasHeadrest, BaseType = s.BaseType, WheelType = s.WheelType, Color = s.Color };
    private DeskSpecificationDto MapDesk(DeskSpecification s) => new DeskSpecificationDto { Id = s.Id, Width = s.Width, Depth = s.Depth, Height = s.Height, Material = s.Material, MaxWeight = s.MaxWeight, IsHeightAdjustable = s.IsHeightAdjustable, HasCableManagement = s.HasCableManagement, HasCupHolder = s.HasCupHolder, HasHeadphoneHook = s.HasHeadphoneHook, SurfaceType = s.SurfaceType, Color = s.Color };
    private ChargerSpecificationDto MapCharger(ChargerSpecification s) => new ChargerSpecificationDto { Id = s.Id, ChargerType = s.ChargerType, TotalWattage = s.TotalWattage, PortCount = s.PortCount, UsbCPorts = s.UsbCPorts, UsbAPorts = s.UsbAPorts, MaxOutputPerPort = s.MaxOutputPerPort, FastChargingProtocols = s.FastChargingProtocols, CableIncluded = s.CableIncluded, HasFoldablePlug = s.HasFoldablePlug, Technology = s.Technology, Color = s.Color };
    private ConsoleSpecificationDto MapConsole(ConsoleSpecification s) => new ConsoleSpecificationDto { Id = s.Id, Processor = s.Processor, Graphics = s.Graphics, RAM = s.RAM, Storage = s.Storage, OpticalDrive = s.OpticalDrive, MaxResolution = s.MaxResolution, MaxFrameRate = s.MaxFrameRate, HDRSupport = s.HDRSupport, Connectivity = s.Connectivity, HasEthernet = s.HasEthernet, WifiVersion = s.WifiVersion, BluetoothVersion = s.BluetoothVersion };
    private HandheldSpecificationDto MapHandheld(HandheldSpecification s) => new HandheldSpecificationDto { Id = s.Id, Processor = s.Processor, Graphics = s.Graphics, RAM = s.RAM, Storage = s.Storage, Display = s.Display, BatteryLife = s.BatteryLife, Weight = s.Weight, OperatingSystem = s.OperatingSystem, Connectivity = s.Connectivity, WifiVersion = s.WifiVersion, BluetoothVersion = s.BluetoothVersion };
    private HubSpecificationDto MapHub(HubSpecification s) => new HubSpecificationDto { Id = s.Id, HubType = s.HubType, PortCount = s.PortCount, UsbAPorts = s.UsbAPorts, UsbCPorts = s.UsbCPorts, HdmiPorts = s.HdmiPorts, DisplayPorts = s.DisplayPorts, EthernetPort = s.EthernetPort, SdCardReader = s.SdCardReader, AudioJack = s.AudioJack, MaxDisplays = s.MaxDisplays, MaxResolution = s.MaxResolution, PowerDelivery = s.PowerDelivery, Color = s.Color };
    private MemoryCardSpecificationDto MapMemoryCard(MemoryCardSpecification s) => new MemoryCardSpecificationDto { Id = s.Id, Capacity = s.Capacity, CardType = s.CardType, SpeedClass = s.SpeedClass, ReadSpeed = s.ReadSpeed, WriteSpeed = s.WriteSpeed, Warranty = s.Warranty, Waterproof = s.Waterproof, Shockproof = s.Shockproof };
    private MicrophoneSpecificationDto MapMicrophone(MicrophoneSpecification s) => new MicrophoneSpecificationDto { Id = s.Id, MicrophoneType = s.MicrophoneType, PolarPattern = s.PolarPattern, Frequency = s.Frequency, SampleRate = s.SampleRate, Sensitivity = s.Sensitivity, Connectivity = s.Connectivity, Connection = s.Connection, HasShockMount = s.HasShockMount, HasPopFilter = s.HasPopFilter, HasRgb = s.HasRgb, Color = s.Color };
    private MousePadSpecificationDto MapMousePad(MousePadSpecification s) => new MousePadSpecificationDto { Id = s.Id, Width = s.Width, Height = s.Height, Thickness = s.Thickness, Material = s.Material, SurfaceType = s.SurfaceType, BaseType = s.BaseType, HasRgb = s.HasRgb, IsWashable = s.IsWashable, Color = s.Color };
    private NetworkHardwareSpecificationDto MapNetwork(NetworkHardwareSpecification s) => new NetworkHardwareSpecificationDto { Id = s.Id, DeviceType = s.DeviceType, WifiStandard = s.WifiStandard, MaxSpeed = s.MaxSpeed, Frequency = s.Frequency, EthernetPorts = s.EthernetPorts, AntennaCount = s.AntennaCount, HasUsb = s.HasUsb, SecurityProtocol = s.SecurityProtocol, Coverage = s.Coverage };
    private PowerBankSpecificationDto MapPowerBank(PowerBankSpecification s) => new PowerBankSpecificationDto { Id = s.Id, Capacity = s.Capacity, TotalWattage = s.TotalWattage, PortCount = s.PortCount, UsbCPorts = s.UsbCPorts, UsbAPorts = s.UsbAPorts, InputPorts = s.InputPorts, MaxOutputPerPort = s.MaxOutputPerPort, FastChargingProtocols = s.FastChargingProtocols, RechargingTime = s.RechargingTime, HasDisplay = s.HasDisplay, Weight = s.Weight, Color = s.Color };
}
...
{
    // Ignore Core Props handled by Logic
    ...
    // Ignore ALL specs in main product map (handled by Handlers)
