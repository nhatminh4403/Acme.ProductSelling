...
namespace Acme.ProductSelling.Web.Middleware
{
    public class DatabaseLimitMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context, IStringLocalizer<ProductSellingResource> localizer)
        {
            ...
            {
                await ShowLimitReachedPage(context, localizer);
                return;
            }
        ...
        private async Task ShowLimitReachedPage(HttpContext context, IStringLocalizer<ProductSellingResource> L)
        {
            ...
            var html = $@"
<!DOCTYPE html>
<html lang='{(L["CultureName"] ?? "en")}'>
<head>
    <meta charset='UTF-8'>
    <meta name='viewport' content='width=device-width, initial-scale=1.0'>
    <title>{L["DatabaseLimit:Title"]}</title>
    <style>
        /* ... styles truncated for brevity in replacement chunk but I'll keep them ... */
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        body {{ font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, sans-serif; display: flex; justify-content: center; align-items: center; min-height: 100vh; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 20px; }}
        ...
        .info-box {{ background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white; padding: 1.5rem; border-radius: 10px; margin: 1.5rem 0; box-shadow: 0 4px 15px rgba(0,0,0,0.1); }}
...
<body>
    <div class='container'>
        <div class='emoji'>💸</div>
        <div class='student-badge'>👨‍🎓 {L["DatabaseLimit:Badge"] ?? "Student Project on Azure Free Tier"}</div>
        <h1>{L["DatabaseLimit:Title"]}</h1>
        <p class='subtitle'>{L["DatabaseLimit:Subtitle"].Value.Replace("{Month}", _currentMonth)}</p>
        
        <p class='casual-text'>
            {L["DatabaseLimit:CasualText1"].Value.Replace("{Month}", _currentMonth)}
            {L["DatabaseLimit:CasualText2"]}
        </p>
        
        <p class='casual-text'>
            {L["DatabaseLimit:CasualText3"].Value.Replace("{Month}", _nextMonth)}
            {L["DatabaseLimit:CasualText4"]}
        </p>
 
        <div class='info-box'>
            <strong><span class='icon'>⏰</span>{L["DatabaseLimit:ResumesOn"]}</strong>
            <div class='reset-date'>{_resetDateInfo}</div>
        </div>
 
        <div class='details'>
            <h3><span class='icon'>🎓</span>{L["DatabaseLimit:WhyHappened"]}</h3>
            <p><strong>• </strong>{L["DatabaseLimit:Reason1"]}</p>
            <p><strong>• </strong>{L["DatabaseLimit:Reason2"]}</p>
            <p><strong>• </strong>{L["DatabaseLimit:Reason3"]}</p>
            <p><strong>• </strong>{L["DatabaseLimit:Reason4"]}</p>
        </div>
 
        {(string.IsNullOrEmpty(_fullErrorMessage) ? "" : $@"
        <details style='margin-top: 1.5rem;'>
            <summary style='cursor: pointer; color: #667eea; font-weight: bold; padding: 0.5rem;'>
                🔍 {L["DatabaseLimit:TechnicalDetails"]}
            </summary>
        ...
        <div class='footer'>
            <p>{L["DatabaseLimit:Footer1"]}</p>
            <p><strong>{L["DatabaseLimit:Footer2"].Value.Replace("{Month}", _nextMonth)}</strong></p>
            <p style='margin-top: 1rem; font-size: 0.8rem;'>
                {L["DatabaseLimit:Footer3"]}
            </p>
        </div>
    </div>
</body>
